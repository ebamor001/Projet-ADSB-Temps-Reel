...
for j = 1:length(eb_n0)
    ...
    while nb_erreurs < 1000  % au moins 100 erreurs pour validité statistique
        ...
        % --- Réalignement via convolution propre (équivaut à rl(delta_t_estime + length(sp) + 1:end)) ---
        %h_sync = zeros(1, length(sl));
        %h_sync(delta_t_estime + 1) = 1;
        %yl_sync = conv(sl, h_sync, 'same');
        
        yl_sync = yl(delta_t_estime + length(sp) + 1:end); % Décaler le signal de delta_t
